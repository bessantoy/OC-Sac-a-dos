...
BranchingStrategies& operator++(BranchingStrategies& e) {
  ...
  e = BranchingStrategies(
      static_cast<std::underlying_type<BranchingStrategies>::type>(e) + 1);
  return e;
}
...
int main(int argc, char** argv) {
  if (argc < 2) {
    ...
    cerr << "Usage: a heuristic/exactMode implies comparisons of exact methods "
            "(for small instances), whereas no parameter implies heuristic "
            "search"
         << endl;
